...
func TestIsValidRepoScope(t *testing.T) {
	tests := []struct {
		...
		{"myorg/*", true},
		{"myorg/prefix*", true},
		{"myorg/my_repo-1", true},
		{"github", false},                   // no slash — not a scope (use isValidRepoOwner)
		{"github/repo/extra", false},        // too many segments
		{"GITHUB/repo", false},              // uppercase not allowed
		{"github/", false},                  // empty repo part
		{"github/*extra*", false},           // multiple wildcards
		{"github/repo*extra", false},        // wildcard not at end
		{"github/repo.", false},             // trailing dot with wildcard prefix check
		{"/repo", false},                    // empty owner
		{"github/.*", false},                // leading dot before wildcard
	}
	for _, tt := range tests {
		t.Run(tt.scope, func(t *testing.T) {
			assert.Equal(t, tt.valid, isValidRepoScope(tt.scope), "scope=%q", tt.scope)
		})
	}
}

// ──────────────────────────────────────────────────────────────────────────────
// isValidRepoOwner / isValidRepoName / isValidTokenString
// ──────────────────────────────────────────────────────────────────────────────
...
func TestIsValidRepoOwner(t *testing.T) {
	tests := []struct {
		...
		{"", false},
		{"My-Org", false},              // uppercase
		{"org@123", false},             // invalid char
	}
	for _, tt := range tests {
		t.Run(tt.owner, func(t *testing.T) {
			assert.Equal(t, tt.valid, isValidRepoOwner(tt.owner), "owner=%q", tt.owner)
		})
	}
}
...
func TestIsValidRepoName(t *testing.T) {
	tests := []struct {
		...
		{"", false},
		{"MyRepo", false},   // uppercase
		{"repo.name", false}, // dot not allowed
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.valid, isValidRepoName(tt.name), "repoName=%q", tt.name)
		})
	}
}
...
func TestValidateStringArrayField(t *testing.T) {
	tests := []struct {
		...
		{"valid array", []interface{}{"a", "b"}, false, false},
		{"valid non-empty array", []interface{}{"a"}, true, false},
		{"empty array optional", []interface{}{}, false, false},
		{"empty array required", []interface{}{}, true, true},
		{"not an array", "string", false, true},
		{"not an array required", 42, true, true},
		{"empty string entry", []interface{}{""}, false, true},
		{"whitespace string entry", []interface{}{"  "}, false, true},
		{"non-string entry", []interface{}{42}, false, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateStringArrayField("test-field", tt.raw, tt.requireNonEmpty)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestIsValidAllowOnlyReposValue(t *testing.T) {
	tests := []struct {
		...
		{"string all", "all", true},
		{"string ALL uppercase", "ALL", true},
		{"string public", "public", true},
		{"string  public with spaces", "  public  ", true},
		{"string unknown", "unknown", false},
		{"valid scope array", []interface{}{"github/repo"}, true},
		{"empty scope array", []interface{}{}, false},
		{"invalid scope array", []interface{}{"BAD_SCOPE"}, false},
		{"nil", nil, false},
		{"integer", 42, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.valid, IsValidAllowOnlyReposValue(tt.repos), "repos=%v", tt.repos)
		})
	}
}
