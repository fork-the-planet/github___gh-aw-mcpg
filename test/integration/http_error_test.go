...
func TestHTTPError_ServerError(t *testing.T) {
	...
	conn, err := mcp.NewHTTPConnection(ctx, "test-server", mockServer.URL, nil, nil, "")
	if err == nil {
		conn.Close()
...
func TestHTTPError_ConnectionRefused(t *testing.T) {
	...
	conn, err := mcp.NewHTTPConnection(ctx, "test-server", "http://"+addr, nil, nil, "")
	if err == nil {
		conn.Close()
...
func TestHTTPError_IntermittentFailure(t *testing.T) {
	...
	conn1, err1 := mcp.NewHTTPConnection(ctx1, "test-server", mockServer.URL, nil, nil, "")
	if err1 == nil {
		conn1.Close()
	...
	conn2, err2 := mcp.NewHTTPConnection(ctx2, "test-server", mockServer.URL, nil, nil, "")
	if err2 != nil {
...
func TestHTTPError_RequestFailure(t *testing.T) {
	...
	ctx := context.Background()
	conn, err := mcp.NewHTTPConnection(ctx, "test-server", mockServer.URL, map[string]string{"X-Test": "test"}, nil, "")
