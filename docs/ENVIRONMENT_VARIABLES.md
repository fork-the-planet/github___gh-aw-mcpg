...
| `MCP_GATEWAY_TOOL_TIMEOUT` | Tool invocation timeout in seconds. Used as fallback when `gateway.toolTimeout` is not set in the stdin JSON config. Accepts integers in the range `10`–`600`. Priority: stdin `gateway.toolTimeout` > this env var > built-in default. | `60` |
